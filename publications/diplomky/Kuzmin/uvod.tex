...
kte\-rý\-mi pracuje metoda GUHA a kte\-rý\-mi se zde budeme podrobnìji zabývat.

\subsection[Asociaèní pravidla \uv{nákupních košíkù}]{Asociaèní pravidla -- analýza nákupních košíkù}
\label{asociacnipravidla}
\paragraph{}
\uv{Klasické} asociaèní pravidlo je definováno Aggravalem (\cite{aggraval})
...
která hledá vztahy, jenž jsou zobecnìním \uv{klasických} asociaèních pravidel definovaných Aggravalem v \cite{aggraval}.
...
\paragraph{}
Nìkteré úlohy pro dobývání znalostí navíc nejdou jednoduše provést nad \uv{plochou}
...
omezení na jednu tabulku dat a umožnìní do\-bý\-vá\-ní znalostí z více relaènì
...
\begin{quote}
\uv{LISp-Miner je akademický projekt pro podporu výzkumu a výu\-ky dobývání znalostí z databází. Je vhodný zejména pro výuku studentù,
...
jak je uvedeno v dokumentu \uv{Implementace Ferda}, který je souèástí dokumentace studentského projektu
Ferda DataMiner.
\paragraph{}
...
\subsection{Diplomová práce \uv{Experimentální GUHA pro\-ce\-du\-ry}}
\paragraph{}
Zásadním pøínosem pro prostøedí Ferda DataMiner se stala diplomová práce T.Kuchaøe
\uv{Experimentální GUHA procedury}, která pøinesla novou implementaci krabièek šesti
...
Booleovský atribut je formule splòu\-jí\-cí následující podmínky.
\paragraph{}
...
odpovídají pod\-mín\-ce na ètyøpolní tabulku. Na základì ètyøpolní tabulky se urèuje,
...
\uv{100\% zkoumaných objektù splòujících $\phi$ splòuje také $\psi$} 
nebo ménì formálnì \uv{$\phi$ implikuje $\psi$ na 100\%}. 
Podmínka na $a$ zajišuje, že v datové matici $M$ (resp. $M/\chi$) je dostateèný poèet 
...
Bit 1 reprezentuje \uv{objekt má tuto hodnotu}, zatímco bit 0 znamená
\uv{objekt nemá tuto hodnotu}.
\paragraph{}
...
sémantickými typy byla realizována v práci \cite{kuchar} -- \uv{Experi\-men\-tál\-ní
GUHA procedury}.
...
po\-hla\-ví nebo oblíbená pøíchu zmrzliny.
...
implementována pouze krabièka pro transformaci atributu \uv{Each value one category}, která, jak plyne
z názvu, umí vytvoøit ze zkoumaného datového sloupce atribut obsahující
výètové kategorie pro každou hodnotu ve sloupci.
\paragraph{}
Krabièky pro atributy \uv{Ekvidistanèní intervaly} a \uv{Ekvifrekvenèní inter\-va\-ly} ve formì
...
implentována krabièka \uv{Statický atribut}, která umožòuje pøí\-mou úpravu
...
Ferdy, avšak diplomová práce T.Kuchaøe \uv{Experimentální procedury} podstatnì zmìnila
...
napøíklad \uv{šetrný} nebo naopak \uv{rozhazovaèný}. Takové charakteristiky zkusíme zjistit na základì dat v matici transakcí.
...
provádí na ní nìjaké transformace a jako výsledek vznikají nové \uv{virtuální} sloupce
...
z definice virtuálního atributu neukládájí do databáze),
...
nadefinovaný sloupec by se použil \uv{o úroveò výše} -- pro generaci hypotéz
...
\uv{Hypothesis space explosion}, jak o nìm pojednává T.Karban v \cite{relminer}.
\end{description}
\paragraph{}
