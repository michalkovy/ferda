...
je strojové získávání netriviálních, døíve neznámých
skrytých vztahù z dat získaných pozorováním nebo z libovolnì velkých databází.
Takto nalezené vztahy mohou být reprezentovány ve srozumitelné
podobì s cílem dalšího jejich použití. Zde se budeme zabývat
nalezením vzorcù v podobì asociaèních pravidel a asociaèních pravidel SDS
...
lze dosáhnout pøi kombinaci více druhù. Jako pøíklad mùžeme uvést statistické
...
Podrobnìjší pøehled metod je uvedený napøíklad v diplomové práci M.Ducháèka \cite{duchacek}.

\section{GUHA}
\paragraph{}
GUHA je zkratka pro General Unary Hypotheses Automaton.
Tuto metodu poprvé formuloval Hájek již v roce 1966 (podrobnìji viz \cite{guha66}),
detailnìji pak v \cite{guhabasic}.
\paragraph{}
Jednou z nejdùležitìjších procedur GUHA je procedura ASSOC (podrobnìji v \cite{guhabasic}),
která hledá asociaèní pravidla, která odpovídají statistickým testùm hypotéz. Tyto vznikají
generováním všech možných relevantních otázek specifikovaných uživatelem pomocí zadání úlohy,
ovìøením každé z nich oproti datùm a reportováním
...
\end{table}
\paragraph{}
Výraz $P_{2}, P_{5} \to P_{2}, P_{3}$ je pøíklad asociaèního pravidla, které øíká, že
...
\paragraph{}
\label{assocrules}
Další skupinu asociaèních pravidel popisuje metoda GUHA. Ty jsou obecnìjší než
''klasická'' asociaèní pravidla analyzující nákupní košíky.
Tato metoda byla poprvé formulována v roce 1966 a byla vydána v 1978
...
použitého kvantifikátoru $\approx$, pøípadnì souvisí za pøedpokladu splnìní
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=8cm]{pics/guha.eps}
\end{center}
...
propojených datových tabulek. Navíc, jak uvádí T.Karban v \cite{dateso2005},
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10cm]{pics/battr.eps}
\end{center}
...
TYPE="VKLAD" \& AMOUNT > AVG(AMOUNT)*2 \Rightarrow_{80\%} \nonumber\\
...
VYSOKEINKASO \& SALARY(>15000) \& BYDLISTE="Praha" \Rightarrow_{80\%} \nonumber\\
STATUSUVERU="Dobry"
\end{eqnarray}
