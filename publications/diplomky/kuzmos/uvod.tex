\chapter{Úvod}
V této kapitole jsou uvedeny základní pojmy dobıvání znalostí. Je popsána
...
V èeštinì se pouívá pojem \emph{dobıvání}, \emph{získávání}, \emph{dolování} nebo \emph{vytìování dat}.
...
\paragraph{}
Dobıvání znalostí stojí na pomezí vıpoèetních
statistickıch metod, databázovıch technologii a strojového uèení.
Metodami dobıvání znalostí se dají øešit úlohy deskripce dat, sumarizace,
...
v souvislosti s analızou nákupního košíku. Zde pouijeme jeho popis, jak je uveden J.Rauchem
v \cite{skripta}.
\paragraph{}
...
pravdìpodobnì bude obsahovat mnoinu pøedmìtù $Y$. Na\-pøík\-lad klienti, kteøí
...
\paragraph{}
Pouívají se dvì míry intenzity asociaèního pravidla -- \emph{podpora (support)} a \emph{konfidence (confidence)}.
...
vstupu, kterı je tvoøen analyzovanımi daty a jednoduchım zadáním relevantních otázek,
generuje zadané otázky a ovìøuje je
oproti vstupním datùm. Vıstupem jsou pak všechny relevantní otázky (také hypotézy), které jsou prosté.
Hypotéza je prostá, jestlie je pravdivá 
v analyzované datové matici a zároveò nevyplıvá z jinıch, jednodušších hypotéz.
\paragraph{}
Jednou z nejpouívanìjších GUHA procedur je procedura ASSOC (podrobnìji v \cite{guhabasic}),
která hledá vztahy, jen jsou zobecnìním ''klasickıch'' asociaèních pravidel definovanıch Aggravalem v \cite{aggraval}.
Nejnovìjší implementací ASSOC je procedura 4FT, která je popsána dále v tomto textu.

\subsection{Relaèní dobıvání znalostí}
Následující úvaha o relaèním dobıvání znalostí byla pøevzata z disertaèní práce T.Karbana
uvedené v seznamu pouité literatury jako \cite{relminer}.
\paragraph{}
Existující metody dobıvání znalostí byly pøevánì navreny pro data ulo\-e\-na
...
\begin{quote}
''LISp-Miner je akademickı projekt pro podporu vızkumu a vıuky dobıvání znalostí z databází. Je vhodnı zejména pro vıuku studentù,
pilotní a støednì velké projekty KDD.''
\end{quote}
Vıuka probíhá zejména na Fakultì informatiky a statistiky
Vysoké školy ekonomické v Praze a Matematicko-fyzikální fakultì Univerzity Karlovy
v Praze. LISp-Miner obsahuje implementaci nìkolika procedur pro dobıvání znalostí spolu s moduly pro zadávání úlohy a prezentace vısledkù.
Vzhledem k dlouhé dobì vıvoje prostøedí (ji od roku 1996) znaènì vzrostla komplexnost celého prostøedí
z hlediska uivatelské práce -- kadá dataminingová procedura se napøíklad skládá ze tøí modulù: pro zadání úlohy,
pro samotné generování a pro prohlíení vısledkù. Za jeden z nejvìtších nedostatkù systému LISp-Miner lze povaovat i fakt,
e zadání úloh pro jednotlivé procedury lze sdílet mezi sebou pouze minimálnì -- v tom autor spatøuje jednu z velkıch
vıhod prostøedí Ferda DataMiner popsaného dále, z uivatelského hlediska. Na druhou stranu je tøeba øíct, e vzhledem k nejdelší historii
systému LISp-Miner procedury v nìm implementované jsou relativnì stabilní a odladìné a pøi jejich novıch implementacích v jinıch prostøedích pro dobıvání znalostí
lze vısledky ze systému LISp-Miner povaovat za vzorové.
...
\begin{itemize}
\item	rozšíøit jazyk asociaèních pravidel pro relaèní dobıvání znalostí,
\item	implementovat systém Rel-Miner za pouití bitovıch øetízkù,
\item	experimentovat s  relaèním dataminingem, zkoumat efektivitu a po\-ui\-tel\-nost,
\item	vytyèit monosti dalšího vıvoje.
\end{itemize}

Stávající implementace dataminingovıch procedur a parametrù zadávání úlohy,
které vznikly v rámci diplomové práce T.Kuchaøe, jsou v nìkterıch ohledech inspirovány právì systémem Rel-Miner. Rovnì
...
T.Karbana hlavnì na monost distribuovanosti bìhu re\-laè\-ních i obecnì nerelaèních dataminingovıch
...
kdeto implementace relaèních rozšíøení v rámci této diplomové práce je zamìøená na práci
...
re\-laè\-ní tìby dat poloit základy pro snazší pøidávání re\-laè\-ních rozšíøení pro zbıvající
...
vznikl ja\-ko softwarovı projekt na Matematicko-fyzikální fakultì Univerzity Karlovy.
...
vıše uvedenıch procedur systému LISp-Miner, pøináší vi\-zuál\-ní mo\-del postupu dobıvání
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=15cm]{pics/ferda-tutorial.eps}
\end{center}
...
\end{figure}
Ještì zde uvedeme \emph{modul} z prostøedí Ferda DataMiner. Moduly slouí pro nastavování
parametrù krabièek a pro komunikaci s uivatelem. Moduly pracují nad urèitımi typy krabièek.
Metoda pro statickou úpravu atributu v této práci byla naprogramována jako modul pro interakci s uivatelem.
...
booleovského at\-ri\-bu\-tu jsou popsány T.Karbanem v \cite{relminer}
...
$Ant$ a $Succ$ na mnoinì $\beta$, je-li splnìna podmínka zadaná booleovskım
 atributem $Cond$ na analyzované datové ma\-ti\-ci.
...
\paragraph{}
Typická efektivní realizace GUHA procedury pouívá ''vertikální'' rep\-re\-zen\-ta\-ci
...
Smyslem zadání úlohy je zej\-mé\-na urèení zpùsobu kategorizace tak, aby byl
...
GUHA pracuje s daty z datovıch matic. Ty jsou získávány pøe\-vánì jako data\-bázové
...
\paragraph{}
Monosti práce s kategoriemi se postupnì vyvíjely v implemen\-tacích me\-to\-dy GUHA. Kate\-gorie
mùe bıt dvojího typu: buï interv\-alová -- obsahuje koneè\-nı poèet inter\-valù,
kterı pokrıvá zkoumanou mnoinu dat, anebo vıè\-tová, kde vıèet opìt pok\-rıvá
zkoumanou mnoinu dat. V sys\-témech LISp-Miner a Rel-Miner kate\-gorie
mùe obsa\-hovat buï jenom inter\-valy anebo jenom vıèet. A v rámci dip\-lomové
...
a intervalù v kategorii. Podmínkou pro monost vytváøení inter\-valù v kategorii
...
s atributy a kategorii, avšak pro uivatelskou práci byl z dùvodu prac\-nosti
...
\section{Relaèní asociaèní pravidla a vir\-tuál\-ní atri\-bu\-ty}
\subsection{Úvod}
...
jako vıs\-ledek bìhu nìk\-teré z data\-miningovıch úloh -- konkrét\-nì na\-pøíklad mùeme
pouít míru plat\-nosti hypo\-téz pro jednot\-livé záz\-namy v hlav\-ní tabul\-ce jako
...
Pro kadı objekt z hlavní datové matice se pøiøadí reálné èíslo
...
dat pøed samotnou tìbou dat, pøidáním po\-a\-dova\-nıch sloupcù do hlavní datové matice apod.
Agregaè\-ní atr\-ibut umoòuje pru\-nì mìnit zadání úlohy -- zjis\-tíme-li na\-pøíklad
...
síla nebo platnost hypotézy nad ved\-lej\-ší\-mi datovımi tabulkami a takto
...
atributu je vlastnì vısledkem bìhu dataminingové úlohy nad ved\-lej\-ší datovou
matici. Tato úloha nad ved\-lej\-ší datovou matici (budeme jí øíkat podúloha)
musí probìhnout vícekrát, vdy pouze nad tìmi øádky z vedlejší matice, které se vztahují
ke zkou\-ma\-né\-mu objektu z hlavní matice. Toto lze ilustrovat obrázkem \ref{fig:master-client1}.
...
\paragraph{}
Uvaujeme-li o podúloze jako napøíklad o proceduøe 4ft-Miner, víme, e ta generuje relevantní
...
\paragraph{}
Spustíme-li 4ft-Miner nad relevantními øádky vedlejší datové matice, dos\-ta\-ne\-me
jako vıstup platné i neplatné relevantní otázky (uvaujme na chvíli, e 4ft-Miner
je takto jednoduše upraven). V pøípadì procedury 4ft-Miner mùeme získat booleovskı
...
tedy vektor reálnıch èísel. Po prozkoumání celé vedlejší datové matice zís\-ká\-me poèet
vektorù platnosti relevantních otázek rovnı poètu øádkù v hlav\-ní datové matici --
pro kadého klienta prozkoumáme relevantní otázky generované podúlohou,
které platí na právì jeho transakcích. Poèet virtuálních sloupcù vygenerovanıch virtuálním atributem
(podúlohou 4ft-Miner) je rov\-nı délce vektoru platnosti relevantních otázek. Chceme-li získat
$i$-tı vir\-tuál\-ní sloupec, vezmeme z kadého vektoru hodnotu na $i$-té pozici.
\paragraph{}
Vısledkem bìhu podúlohy je tedy mnoina novıch virtuálních sloupcù,
které jsou pøidány za bìhu k hlavní datové matici a hlavní dataminingová úloha
s nimi pracuje jako s ostatními sloupci.
\subsection{Nárùst sloitosti vıpoètu}
...
popisu získávání virtuálních sloupcù plyne, e pro kadou relevantní otázku
se do hlavní datové matice pøidá virtuální sloupec. Je vidìt, e pøi pouívání
hypotézového atributu v zadání úlohy pro dataminingovou proceduru lze velmi
lehce dosáhnout vırazného zesloitìní pùvodní úlohy a rychlému nárùstu velikosti
zkoumané datové matice. Tato skuteènost vede ke zpomalení bìhu úlohy a ke generování
velkého poètu hypotéz, které øíkají skoro toté, protoe mají velmi podobné hodnoty
...
aby platil na co nejvìtší podmnoinì zkoumanıch dat.
...
hypotézovı atribut pouít v zadání pro další hypotézovı atribut.
\paragraph{}
...
lze jen tìko pop\-sat v pøi\-rozeném jazy\-ce.
Obtínì se interpretují zá\-vislosti na\-lezené na zkou\-manıch datech.
...
Tento dùvod souvisí s pøedchozím -- ne\-jas\-nı vız\-nam vıs\-led\-kù vede k potíim jak
...
Z uvedeného zpù\-sobu genero\-vání virtuálních sloupcù je vidìt, e jejich poèet mùe bıt velkı
a proto úloha nad hlavní matici se zpomalí. Pøed\-stavíme-li si, e pouijeme v zadání
...
Kdy uváíme monost pouití v zadání vícero virtuálních atributu, je vidìt, e sloitost a doba
...
logického programování (ILP)}. Prin\-cip ILP mùe bıt pop\-sán ná\-sledovnì: z data\-báze faktù
a oèekávanıch vısledkù, které jsou roz\-dìleny na klad\-né a zápor\-né pøí\-klady, met\-ody
pracující na základì ILP se snaí odvodit logickı program, kterı dokáe všechny kladné
