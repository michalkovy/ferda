...
existujících modulù prostøedí LISp-Miner. To pøineslo øadu potíí podrobnì
...
jak zpùsob zadávání úlohy, tak i nìkteré èásti procedur. Z uivatelského hlediska se
do relaèní procedury zapojí úloha stejnì jako do nerelaèní a vıstup se zapojuje stejnì
jako z existujících booleovskıch atributù. Z programátorského hlediska se vyuívá
...
z hlavních cílù projektu Ferda DataMiner, protoe takovı pøístup umoòuje rychlejší
...
\subsection{Licence}
Protoe prostøedí Ferda DataMiner je vyvíjeno pod licenci
...
atribitu se bral v potaz fakt, e daleko zajímavìjším atributem pro vyuití
...
ne pouze spojení dvou databazovıch tabulek dle klíèe a spoustìní pouze SQL agergaèních funkcí.
...
dalších atributovıch krabièek a jednoho modulu pro úpravu atributu, bylo rozhodnuto
naimplementovat v rámci agregaèního hypotézového atributu pouze funkènost, která je podrobnìji
...
právì jednu podmnoinu øádkù vedlejší datové matice pro jeden objekt z hlavní
...
diplomové práce zdála jako velmi špatnì navrena z dùvodù pøíliš velké
...
a \emph{další} bitovı øetízek anebo nic v pøípadì vyèerpání monıch sloupcù.
\paragraph{}
...
relevantních otázek. Je moné pøistoupit k jakémukoliv bitovému øetízku na základì klíèe.
\paragraph{}
Oproti tomu navrhovanı zpùsob implementace hypotézového atributu zaruèoval pouze to,
e takovı atribut v okamiku zavolání pøíslušné metody buï vydá další bitovı øetízek
nebo oznámí, e bitové øetízky u vydávat nebude. Není tedy známá a snadno pøístupná
celá mnoina bitovıch øetízku pøed zaèátkem generování (nadúlohou) relevantních otázek.
...
\paragraph{}
Vıhody tohoto øešení ve smyslu škálovatelnosti jsou zøejmé. Podúloha bìí synchronnì
s nadúlohou a v pamìti se uchovává a pøedává pouze jeden bitovı øetízek v jednom okamiku.
...
Nejpodstatnìjším dùsledkem byla nemonost pouívat stávající cache bitovıch
...
nešlo stávající cache pro hypotézovı virtuální atribut pouit. Pouití
...
\paragraph{}
V pøípadì hypotézového virtuálního atributu by nešla pouít stávající krabièka
pro generování koeficientù ''AtomSetting'', protoe pracuje s poadavkem ''poskytni
pøesnì tento bitovı øetízek'', kdyto hypotézovı atribut by umìl zpracovat pouze
poadavek ''poskytni další bitovı øetízek''. Tudí krabièka pro generování koeficientù
vyuitelná pouze pro hypotézovı virtuální atribut by se se musela také naprogramovat.
\paragraph{}
...
bude potøeba implementovat pouze podmnoiny délky jedna -- viz \ref{coefficients}.
...
se hlavní úloha pøíliš nezpomalovala èekáním na bitstring od podúlohy. Protoe
...
pøes ICE bylo bylo tøeba èekat pøi kadém kroku  \verb+foreach+ cyklu),
...
protoe bitstringy jim generované jsou pouívany ve vnitøním \verb+foreach+ cyklu,
a v pøípadì nepouívaní cache to znamená, e uvnitø tohoto cyklu se \emph{pokadé} spustí
...
\paragraph{}
Protoe bìné poèty vygenerovanıch relevantních otázek v procedurách GUHA se pohybují v øádech
...
V rámci implementace funkènosti tohoto atributu byly vyuity algoritmy
...
pro vyuití v prostøedí Ferda DataMiner. Autor dané diplomové práce
...
disjunktnost intervalù, jak je poadováno pøi tvorbì úlohy pro dataminingovou
