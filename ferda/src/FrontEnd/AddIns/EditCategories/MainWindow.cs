...
namespace Ferda
{
    namespace FrontEnd.AddIns.EditCategories
    {
        public partial class MainListView : System.Windows.Forms.Form, Ferda.FrontEnd.IIconProvider
        {
            ...
            {
                foreach (Category multiSet in smartList.MultiSets)
                {
                    ...
                    /*  //xcat
                      ...
                      //frequency
                      item.SubItems.Add(multiSet.GetFrequency().ToString());*/
            ...
            private void OnSelectedIndexChanged(object sender, EventArgs e)
            {
                ...
                if (view.SelectedItems.Count > 0)
                {
                    if (view.SelectedItems.Count > 1)
                    {
                        ...
                        if (bigDataList.SameTypeMultiSets(indexes))
                        {
            ...
            /// <summary>
            /// Moves the selected items up the list
            /// </summary>
            /// <param name="sender"></param>
            /// <param name="e"></param>
            private void Up_Click(object sender, EventArgs e)
            {
                ...
                ListViewItem tempItem1 = new ListViewItem();

                this.CategoriesListView.Sorting = SortOrder.None;
                foreach (int item in this.CategoriesListView.SelectedIndices)
                {
                    //if the item is not on the top of the list
                    if ((this.CategoriesListView.Items[item].Index) > 0)
                    {
                        tempItem = this.CategoriesListView.Items[item - 1];

                        this.CategoriesListView.Items[item - 1].Remove();

                        tempItem1 = this.CategoriesListView.Items[item - 1];
                        this.CategoriesListView.Items[item - 1].Remove();

                        this.CategoriesListView.Items.Insert(item - 1, tempItem1);
                        this.CategoriesListView.Items.Insert(item, tempItem);
                    }
                }
            ...
            /// <summary>
            /// Move the selected items down the list
            /// </summary>
            /// <param name="sender"></param>
            /// <param name="e"></param>
            private void Down_Click(object sender, EventArgs e)
            {
                ...
                {
                    //if the item is not on the top of the list
                    if (item < (this.CategoriesListView.Items.Count - 1))
                    {
                        //next item
                        tempItem = this.CategoriesListView.Items[item + 1];

                        //this item
                        tempItem1 = this.CategoriesListView.Items[item];

                        //remove next item
                        this.CategoriesListView.Items[item + 1].Remove();

                        //remove this item
                        this.CategoriesListView.Items[item].Remove();

                        //insert next item on the place of this item
                        this.CategoriesListView.Items.Insert(item, tempItem);

                        //insert this item on the place of next item
                        this.CategoriesListView.Items.Insert(item + 1, tempItem1);
            ...
            private void Delete_Click(object sender, EventArgs e)
            {
                ...
                {
                    this.bigDataList.RemoveMultiSet((int)item.Tag);
                    item.Remove();
                }
            ...
            /// <param name="sender"></param>
            /// <param name="e"></param>
            private void MainListView_Resize(object sender, System.EventArgs e)
            {
                //   this.CategoriesListView.Width = this.Width;
            }
            ...
            //method to initialize listview using ice
            private FerdaSmartDataList MyIceRun(Modules.CategoriesStruct existingCategories)
            {
                ...
                foreach (DictionaryEntry myEnumCategorySeq in existingCategories.enums)
                {
                    ...
                    {
                        tempArray.Add(entry);
                        //returnList.AddToAllValues(entry);
                    }
                    SingleSet newSet = new SingleSet(tempArray);
                    newMultiset.AddSingleSet(newSet);
                    returnList.AddNewMultiSetDirect(newMultiset);
                    //   tempArray.Clear();
                }
                ...
                foreach (DictionaryEntry myFloatIntervalCategorySeq in existingCategories.floatIntervals)
                {
                    ...
                    foreach (FloatIntervalStruct myFloatIntervalStruct in myFloatIntervalStructSeq)
                    {
                        ...
                        else
                        {
                            ...
                            //   newInterval.intervalType = IntervalType.Float;
                            newMultiset.AddInterval(newInterval);
                        }
                        returnList.AddNewMultiSetDirect(newMultiset);
                    }
            ...
            private CategoriesStruct MyIceRunOut(FerdaSmartDataList dataList)
            {
                ...
                foreach (Category multiSet in dataList.MultiSets)
                {
                    switch (multiSet.CatType)
                    {
                        case CategoryType.Interval:
