            ...
            foreach (PropertyInfo pinfo in creator.Properties)
            {
                if (pinfo.visible)
                {
                    ...
                    { //creating normal property
                        ...
                        {
                            //potrebuju zamezit typum, ktere neumim
                            if (GetNormalType(pinfo.typeClassIceId) != "")
                            {
                                //getting the displayable name of the property
                                SocketInfo si = creator.GetSocket(pinfo.name);
                                ps = new FerdaPropertySpec(si.label, GetNormalType(pinfo.typeClassIceId), false);
            ...
            string propertyName, string typeName)
        {
            switch (typeName)
            {
                ...
                case "System.String":
                    ...
                    else //setting the property for all the selkected boxes
                    {
                        foreach (IBoxModule box in SelectedBoxes)
                        {
                            if (box.TryWriteEnter())
            ...
            realPropertyName, string typeName)
        {
            ...
            switch (typeName)
            {
                case "Ferda.FrontEnd.Properties.OtherProperty":
                    ...
                    {
                        //TODO doresit other property a string sequence - zatim to nefunguje
                        //e.Value = GetOtherPropertyMoreBoxes(realPropertyName);
                    }
                    break;
