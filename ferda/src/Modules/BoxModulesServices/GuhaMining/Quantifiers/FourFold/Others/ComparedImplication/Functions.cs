// Functions.cs - Function objects for the Compared Implication quantifier box module
//
...
namespace Ferda.Modules.Boxes.GuhaMining.Quantifiers.FourFold.Others.ComparedImplication
{
    /// <summary>
    /// Computes the Compared Implication quantifier
    /// </summary>
    /// <remarks>
    /// The formula for the quantifier is 
    /// (ad - bc)/c(a+b). We know this quantifier from the 
    /// article "Domain Ontology Driven Data Mining - A Medical Case Study"
    /// by Yen-Ting Kuo, Andrew Lonie, Liz Sonenberg and Kathy Paizis published in
    /// 2007 ACM SIGKDD workshop on domain driven data mining (DDDM2007), 
    /// August 12, 2007 San Jose, California, USA. Here a motivation for
    /// constructing the quantifier and its semantics can be found.
    /// </remarks>
        ...
        public override double ComputeValue(QuantifierEvaluateSetting param, Current current__)
        {
            return ExceptionsHandler.TryCatchMethodThrow<double>(
                delegate
                {
                    ...
                    {
                        double up = (table.A * table.D) - (table.B * table.C);
                        double down = table.C*(table.A + table.B);
                        return up / down;
                    }
                },
