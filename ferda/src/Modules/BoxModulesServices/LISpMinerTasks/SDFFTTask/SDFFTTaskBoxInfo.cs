...
{
	class SDFFTTaskBoxInfo : Ferda.Modules.Boxes.BoxInfo
	{
		...
		{
			SDFFTTaskFunctionsI Func = (SDFFTTaskFunctionsI)boxModule.FunctionsIObj;
			switch (propertyName)
			{
				...
				case "MissingsType":
					return new Ferda.Modules.StringTI(MissigTypeEnum.Deleting.ToString());
				default:
		...
		public override void RunAction(string actionName, BoxModuleI boxModule)
		{
			...
			{
				case "Run":
					((SDFFTTaskFunctionsI)boxModule.FunctionsIObj).RunActionRun();
					break;
				default:
