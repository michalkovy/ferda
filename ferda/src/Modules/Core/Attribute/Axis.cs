// Axis.cs - essential methods for working with the attribute
//
...
namespace Ferda.Guha.Attribute
{
    ...
    /// for the attribute or bit string operations with the attribute
    /// </summary>
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                if (row[0] is DBNull)
                {
                    if (_attribute.NullContainingCategory != null)
                    	result[_attribute.NullContainingCategory] += Convert.ToInt32(row[1]);
                }
                else
                {
                    item = (T)row[0];
                    if (_enumValues.TryGetValue(item, out categoryName))
                    {
                        ...
                        result[categoryName] += Convert.ToInt32(row[1]);
                    }
                    else if (
                        _intervals.TryGetValue(new Interval<T>(item, BoundaryEnum.Closed, item, BoundaryEnum.Closed, _attribute),
                                               out categoryName))
