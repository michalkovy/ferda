        ...
        public static object[] GenerateIntervals(int intervals, object[] sortedValues)
        {
            if ((sortedValues == null) || (sortedValues.Length == 0))
                throw new ArgumentException(
                    "The reference to the array of sorted values is null or contains no elements.", "sortedValues");

            if (intervals < 1)
                throw new ArgumentOutOfRangeException("intervals", intervals,
                                                      "The number of intervals must be at least 1.");
            ...
            if (dataArray.Count < intervals)
                throw new ArgumentOutOfRangeException("intervals", intervals,
                                                      "The requested number of intervals is less than the total number of different values.");
                                        ...
                                        ResultCache cache)
        {
            ...
            while (!leftStop || !rightStop)
            {
                if (!leftStop)
                {
                    ...
                    leftTmpResult =
                        FindSplit(dataArray, new Interval(bounds.Left, leftSplit), leftIntervals, optimum, cache);
                    rightTmpResult =
                        FindSplit(dataArray, new Interval(leftSplit, bounds.Right), intervals - leftIntervals, optimum,
                                  cache);
                ...
                if (!rightStop)
                {
                    // find solution for left and right part
                    leftTmpResult =
                        FindSplit(dataArray, new Interval(bounds.Left, rightSplit), leftIntervals, optimum, cache);
                    rightTmpResult =
                        FindSplit(dataArray, new Interval(rightSplit, bounds.Right), intervals - leftIntervals, optimum,
                                  cache);
